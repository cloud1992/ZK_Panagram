...
contract PanagramTest is Test {
    ...
    bytes32 INCORRECT_GUESS = bytes32(uint256(keccak256("test")) % FIELD_MODULUS);
    bytes proof;
    ...
    function _getProof(bytes32 guess, bytes32 correctAnswer, address _user) internal returns (bytes memory _proof) {
        ...
        inputs[0] = "node";
        inputs[1] = "js-scripts/generateProof.mjs";
        inputs[2] = vm.toString(guess);
        inputs[3] = vm.toString(bytes32(uint256(uint160(_user))));
        inputs[4] = vm.toString(keccak256(abi.encodePacked(correctAnswer)));
